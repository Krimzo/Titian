...
void titian::GUISectionMaterialEditor::render_gui()
{
    ...
    if ( im::Begin( "Material Editor" ) )
    {
        ...
        bool should_rotate_cam = was_focused;
...
void titian::GUISectionMaterialEditor::display_materials( Scene& scene )
{
    if ( im::BeginPopupContextWindow( "NewMaterial", ImGuiPopupFlags_MouseButtonMiddle ) )
    {
        ...
        String name = gui_input_continuous( "##CreateMaterialInput" );
        if ( !name.empty() && !scene.materials.contains( name ) )
        {
            ...
            {
                String filter = gui_input_continuous( "Search###NewTextureMaterial" );
    ...
    String filter = gui_input_continuous( "Search###MeterialEditor" );
...
void titian::GUISectionMaterialEditor::render_selected_material( Material& material, Int2 viewport_size )
{
    ...
    if ( *shaders )
    {
        ...
        shaders->upload( cb );
        Mesh const& mesh = scene.default_meshes.cube;
