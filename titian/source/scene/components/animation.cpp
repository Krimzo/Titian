...
void titian::Animation::deserialize( Serializer const& serializer )
{
...
int titian::Animation::get_index( float time ) const
{
    if ( meshes.empty() )
        return -1;
...
titian::Mesh* titian::Animation::get_mesh( Scene const& scene, float time ) const
{
    if ( meshes.empty() )
        return nullptr;
    if ( animation_type == AnimationType::SKELETAL )
        return scene.helper_get_mesh( meshes[0] );
    int index = get_index( time );
    return scene.helper_get_mesh( meshes[index] );
}

void titian::Animation::update( Scene const& scene, float current_time )
{
...
void titian::Animation::bind_matrices( int slot ) const
{
...
void titian::Animation::upload_matrices()
{
    ...
    kl::GPU const& gpu = AppLayer::get().gpu;
    size_t count = kl::min( m_final_matrices.size(), (size_t) MAX_BONE_COUNT );
    gpu.write_to_buffer( m_matrices_buffer, m_final_matrices.data(), count * sizeof( Float4x4 ) );
}

void titian::Animation::update_node_hierarchy( float time_ticks, Mesh const& mesh, SkeletonNode const& skel_node, AnimationNode const& anim_node, Float4x4 const& parent_transform )
{
    ...
    {
        aiVector3f translation = interpolate_translation( time_ticks, anim_node.channel_index );
        aiQuaternion rotation = interpolate_rotation( time_ticks, anim_node.channel_index );
        aiVector3f scaling = interpolate_scaling( time_ticks, anim_node.channel_index );
...
aiVector3f titian::Animation::interpolate_translation( float time_ticks, int channel_index ) const
{
...
aiVector3f titian::Animation::interpolate_scaling( float time_ticks, int channel_index ) const
{
...
{
    aiMatrix4x4 matrix{ scaling, rotation, translation };
    Float4x4 result;
    for ( int i = 0; i < 4; i++ )
