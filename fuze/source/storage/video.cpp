...
titian::Video::Video( StringRef const& path )
    : m_path( path )
{
...
void titian::Video::store_frame( float time, bool wait )
{
    cache_frames( time );
    ...
    int frame_index = int( time * m_fps );
    m_frames_lock.lock();
...
void titian::Video::cache_frames( float time )
{
    ...
    {
        int frames_per_section = int( BUFFERING_LENGTH * m_fps );
        int frames_per_core = frames_per_section / SECTION_THREAD_COUNT;
        ...
        for ( int core_index = 0; core_index < SECTION_THREAD_COUNT; core_index++ )
        {
            queue.add_task( [this, section_index, frames_per_section, frames_per_core, core_index]
            {
                ...
                int start_index = section_index * frames_per_section + core_index * frames_per_core;
                int end_index = start_index + frames_per_core;
...
titian::Vector<titian::Vector<titian::Pair<bool, float>>> titian::Video::buffering_sections() const
{
    ...
    for ( auto& section : m_buffering_sections )
    {
