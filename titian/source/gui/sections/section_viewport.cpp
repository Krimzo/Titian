...
void titian::GUISectionViewport::render_gui()
{
    ...
    if ( im::Begin( "Viewport", nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse ) )
    {
        ...
        {
            String name = Scene::generate_unique_name( entity_type.value(), scene.entities() );
            Float3 position = get_mouse_3d();
        ...
        ImVec2 viewport_max = im::GetWindowPos() + im::GetWindowSize();
        if ( im::IsWindowFocused() && im::IsMouseHoveringRect( im::GetWindowPos(), viewport_max ) )
        {
            ...
            {
                Int2 rect_selection_first = m_rect_selection_first.value();
                if ( im::IsMouseDown( ImGuiMouseButton_Left ) )
                {
                    Int2 rect_selection_last = window_mouse_position();
                ...
                {
                    Int2 rect_selection_last = window_mouse_position();
                    Set<uint32_t> entity_ids = read_id_texture( rect_selection_first, rect_selection_last );
                    ...
                    {
                        uint32_t counter = 0;
                        for ( auto& [name, _] : scene.entities() )
                        {
                            counter += 1;
        ...
        Set<Entity*> entities;
        for ( auto& sel_ent : editor_layer.selected_entities )
        {
...
{
    ImVec2 window_position = im::GetWindowPos();
    ImVec2 mouse_position = im::GetMousePos();
    float tab_size = im::GetWindowContentRegionMin().y;
    return {
...
titian::Float2 titian::GUISectionViewport::window_to_ndc( Int2 coords ) const
{
    ImVec2 window_size = im::GetWindowContentRegionMax() - im::GetWindowContentRegionMin();
    return {
...
titian::Set<uint32_t> titian::GUISectionViewport::read_id_texture( Int2 first_coords, Int2 last_coords )
{
    ...
    Int2 size = max_coords - min_coords;
...
titian::Opt<titian::Float3> titian::GUISectionViewport::read_depth_texture( Int2 coords )
{
    kl::GPU& gpu = AppLayer::get().gpu;
    Scene& scene = GameLayer::get().scene();
    ...
    float depth = 0.0f;
    gpu.map_read_resource( main_camera->depth_staging.texture, [&]( byte* ptr, uint32_t pitch )
    {
...
void titian::GUISectionViewport::render_gizmos( Set<Entity*> const& entities )
{
    EditorLayer& editor_layer = EditorLayer::get();
    GameLayer& game_layer = GameLayer::get();
    AppLayer& app_layer = AppLayer::get();
    ...
    float viewport_tab_height = im::GetWindowContentRegionMin().y;
    Float2 viewport_position = { im::GetWindowPos().x, im::GetWindowPos().y + viewport_tab_height };
    Float2 viewport_size = { im::GetWindowWidth(), im::GetWindowHeight() };
    ...
    Float4x4 view_matrix = kl::transpose( camera->view_matrix() );
    Float4x4 projection_matrix = kl::transpose( camera->projection_matrix() );
    ...
    if ( !m_was_using )
    {
        ...
        if ( im::IsKeyDown( ImGuiKey_LeftAlt ) )
        {
            ...
            for ( String const& entity_name : editor_layer.selected_entities )
            {
                Entity* entity = scene.helper_get_entity( entity_name );
                if ( !entity )
                    continue;

                String new_name = Scene::generate_unique_name( entity_name, scene.entities() );
