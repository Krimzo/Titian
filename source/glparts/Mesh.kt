...
class Mesh : GLObject, Serializable {
    private var vertices: Array<Vertex> = Array(0) { Vertex() }
    ...
    constructor(holder: NameHolder, name: String, context: GLContext, vertices: Array<Vertex>) : super(holder, name, context) {
        this.vertices = vertices.copyOf()
        var maxDistance = 0f
        for (vertex in vertices) {
            maxDistance = maxDistance.coerceAtLeast(vertex.world.length)
        }
        maxRadius = maxDistance

        val result = generateMesh(vertices)
        result.first?.let { vao = it }
        result.second?.let { vbo = it }
    }

    constructor(holder: NameHolder, name: String, context: GLContext, filePath: String)
            : this(holder, name, context, FileHelper.parseMeshFile(filePath) ?: Array(0) { Vertex() })
    ...
    companion object {
        ...
        fun generateScreenMesh(context: GLContext): Mesh {
            return Mesh(NameHolder(), "Screen Mesh", context, arrayOf(
                Vertex(Float3(-1.0f, -1.0f, 0.5f)), Vertex(Float3(-1.0f, 1.0f, 0.5f)), Vertex(Float3(1.0f, 1.0f, 0.5f)),
                Vertex(Float3(1.0f, 1.0f, 0.5f)), Vertex(Float3(1.0f, -1.0f, 0.5f)), Vertex(Float3(-1.0f, -1.0f, 0.5f))
            ))
        }

        fun generateFrustumMesh(context: GLContext): Mesh {
            ...
            return Mesh(NameHolder(), "Frustum Mesh", context, arrayOf(
        ...
        fun generateGridMesh(context: GLContext, size: Int): Mesh {
            ...
            return Mesh(NameHolder(), "Grid Mesh", context, vertices)
        }
    }
}