...
titian::SandboxPieceSpheres::SandboxPieceSpheres( TitianEditor& editor, int size )
    : SandboxPiece( editor )
{
    this->size = size;
}
...
void titian::SandboxPieceSpheres::setup_self()
{
    ...
    for ( int y = 0; y < size; y++ )
    {
        for ( int x = 0; x < size; x++ )
        {
            ...
            String animation_name = "sphere";
            String counter_id = kl::format( std::setw( 3 ), std::setfill( '0' ), sphere_counter );
            String material_name = kl::format( "sphere_mat_", counter_id );
            String entity_name = kl::format( "Sphere", counter_id );
