...
export void render_scene(EditorState* state)
{
    ...
    public:
        ...
        kl::Float4x4 vp_light_matrices[kl::DirectionalLight::CASCADE_COUNT];
    } vs_data = {};

    vs_data.vp_matrix = state->scene->camera->matrix();

    if (state->scene->directional_light) {
        ID3D11ShaderResourceView* dir_light_views[kl::DirectionalLight::CASCADE_COUNT] = {};
        ...
        state->gpu->context()->PSSetShaderResources(1, kl::DirectionalLight::CASCADE_COUNT, dir_light_views);
    }
