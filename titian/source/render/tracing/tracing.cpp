...
void titian::TracingTextureCube::load_2D( kl::Image const& image )
{
    for ( auto& face : faces )
        face = image;
}

titian::RGB titian::TracingTextureCube::sample( Float3 const& direction ) const
{
    auto [x, y, z] = kl::normalize( direction );
    auto [ax, ay, az] = kl::abs( Float3( x, y, z ) );
    auto [u, v] = Float2();
...
titian::RGB titian::TracingCamera::sample_background( Float3 const& direction ) const
{
    if ( skybox )
    {
...
titian::TracingScene::TracingScene( Scene& scene, Int2 resolution )
{
    std::mutex lock;
...
titian::Opt<titian::TracingPayload> titian::TracingScene::trace( kl::Ray const& ray, kl::Triangle const* blacklist ) const
{
    Float3 intersection;
...
titian::Opt<titian::TracingEntity> titian::TracingScene::convert_entity( Scene const& scene, Entity const& entity )
{
    Animation* animation = scene.helper_get_animation( entity.animation_name );
    if ( !animation )
        return std::nullopt;

    Mesh* mesh = animation->get_mesh( scene, AppLayer::get().timer.elapsed() );
    if ( !mesh )
        return std::nullopt;
...
titian::TracingMesh titian::TracingScene::convert_mesh( Scene const& scene, Mesh const& mesh, Float4x4 const& matrix )
{
    TracingMesh result;
...
titian::TracingMesh titian::TracingScene::convert_skel_mesh( Scene const& scene, Mesh const& mesh, Float4x4 const& model_matrix, Vector<Float4x4> const& bone_matrices )
{
    TracingMesh result;
    result.triangles.resize( mesh.vertices.size() / 3 );
    for ( size_t i = 0; i < result.triangles.size(); i++ )
...
titian::TracingMaterial titian::TracingScene::convert_material( Scene const& scene, Material const& material )
{
...
titian::Opt<titian::TracingTextureCube> titian::TracingScene::convert_texture_cube( Texture* texture )
{
    if ( !texture )
        return std::nullopt;

    Opt<TracingTextureCube> result = TracingTextureCube{};
    kl::Image const& image = texture->image;
...
void titian::Tracing::render( Scene& scene )
{
    TracingScene tracing_scene{ scene, RESOLUTION };
