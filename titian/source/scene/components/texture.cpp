...
void titian::Texture::deserialize( Serializer const& serializer )
{
...
void titian::Texture::create_target_view( dx::TargetViewDescriptor* descriptor )
{
    kl::GPU const& gpu = AppLayer::get().gpu;
    target_view = gpu.create_target_view( texture, descriptor );
}

void titian::Texture::create_depth_view( dx::DepthViewDescriptor* descriptor )
{
    kl::GPU const& gpu = AppLayer::get().gpu;
    depth_view = gpu.create_depth_view( texture, descriptor );
}

void titian::Texture::create_shader_view( dx::ShaderViewDescriptor* descriptor )
{
    kl::GPU const& gpu = AppLayer::get().gpu;
    shader_view = gpu.create_shader_view( texture, descriptor );
}

void titian::Texture::create_access_view( dx::AccessViewDescriptor* descriptor )
{
    kl::GPU const& gpu = AppLayer::get().gpu;
...
void titian::Texture::copy_other( dx::Texture const& texture )
{
    kl::GPU const& gpu = AppLayer::get().gpu;
