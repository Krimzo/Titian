...
void setup_skyboxes(state_machine* state)
{
	...
	state->skyboxes["clouds"] = state->gpu->new_shader_view(clouds_skybox);

	auto night_skybox = parse_skybox(state->gpu, "night.png");
	state->skyboxes["night"] = state->gpu->new_shader_view(night_skybox);
}
...
kl::dx::texture parse_skybox(kl::ref<kl::gpu> gpu, const std::string& name)
{
	...
	return gpu->new_texture(box_sides[0], box_sides[1], box_sides[2], box_sides[3], box_sides[4], box_sides[5]);
}
