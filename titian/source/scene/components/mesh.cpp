...
void titian::Mesh::deserialize( Serializer const& serializer )
{
...
void titian::Mesh::load_vertices( Vector<kl::Vertex> const& vertices )
{
...
void titian::Mesh::load_triangles( Vector<kl::Triangle> const& triangles )
{
...
titian::Ref<titian::Mesh> titian::Mesh::clone() const
{
    ...
    Func<void( SkeletonNode*, SkeletonNode* )> rec_helper;
    rec_helper = [&]( SkeletonNode* source, SkeletonNode* target )
    {
        if ( !source || !target )
            return;
