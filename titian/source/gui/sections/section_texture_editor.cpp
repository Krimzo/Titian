...
void titian::GUISectionTextureEditor::display_textures( Scene& scene )
{
    ...
    String filter = gui_input_continuous( "Search###TextureEditor" );
    for ( auto& [texture_name, texture] : scene.textures )
    {
        ...
        if ( im::BeginPopupContextItem( texture_name.data(), ImGuiPopupFlags_MouseButtonRight ) )
        {
            ...
            if ( auto opt_name = gui_input_waited( "##RenameTextureInput", texture_name ) )
            {
                ...
                if ( !new_name.empty() && !scene.textures.contains( new_name ) )
                {
                    ...
                    {
                        if ( material->color_texture_name == texture_name )
                            material->color_texture_name = new_name;
                        if ( material->normal_texture_name == texture_name )
                            material->normal_texture_name = new_name;
                        if ( material->roughness_texture_name == texture_name )
                            material->roughness_texture_name = new_name;
                    }
                    for ( auto& [_, entity] : scene.entities() )
                    {
                        if ( Camera* camera = &entity.as<Camera>() )
                        {
                            if ( camera->skybox_texture_name == texture_name )
                                camera->skybox_texture_name = new_name;
                            if ( camera->target_texture_name == texture_name )
                                camera->target_texture_name = new_name;
                        }
                    }
...
{
    ImVec2 region_size = im::GetContentRegionAvail();
    Int2 image_res = texture.resolution();
