...
template<typename V>
static std::type_info const& variant_type( V const& variant )
{
    return std::visit( []( auto&& value ) -> std::type_info const& { return typeid(value); }, variant );
}
...
void titian::GUISectionScriptingParameters::display_interp_parameter_editor( int script_id, StringRef const& name, InterpScript::Parameter& parameter )
{
    String identifier = kl::format( name, "##", script_id );
...
void titian::GUISectionScriptingParameters::display_node_parameter_editor( int script_id, StringRef const& name, std::type_info const& type, void* ptr )
{
    if ( !ptr )
        return;
