...
titian::StringSet titian::list_files( StringRef const& input )
{
...
bool titian::create_package( StringRef const& input, StringRef const& output_file )
{
    if ( !fs::exists( input ) )
    {
    ...
    StringSet files = list_files( input );
...
bool titian::open_package( StringRef const& input_file, StringRef const& output_dir )
{
    BinarySerializer serializer{ input_file, false };
    if ( !serializer )
    {
