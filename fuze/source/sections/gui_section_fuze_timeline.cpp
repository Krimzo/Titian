...
{
    AppLayer const& app_layer = AppLayer::get();
    VideoLayer const& video_layer = VideoLayer::get();

    kl::Window const& window = app_layer.window;
    ...
    {
        float available_height = im::GetContentRegionAvail().y;
        ...
        if ( im::BeginTable( "##TracksTable", 2, ImGuiTableFlags_Borders | ImGuiTableFlags_Resizable ) )
        {
            ...
            float row_height = (available_height - header_height) / vertical_view;
...
void titian::GUISectionFuzeTimeline::handle_input( int scroll )
{
    ...
    if ( video_layer.can_edit() && im::BeginPopupContextWindow( "##TimelinePopup" ) )
    {
        ...
        else if ( m_editing_media && video_layer.selected_media )
        {
            ...
            if ( video_layer.selected_media->type == MediaType::VIDEO && video_layer.selected_media->has_audio() && im::Button( "Split Audio", { -1.0f, 0.0f } ) )
            {
    ...
    if ( im::IsWindowFocused() )
    {
        ...
        {
            float start_time = video_layer.start_time();
...
void titian::GUISectionFuzeTimeline::render_header( ImVec2 cell_padding, float available_height, float& header_height, ImVec2& total_min, ImVec2& total_max )
{
    VideoLayer& video_layer = VideoLayer::get();
    ...
    String text = kl::format( std::fixed, video_layer.current_time );
    ImVec2 text_size = im::CalcTextSize( text.data() );

    ImVec2 cursor_pos = im::GetCursorScreenPos();
    float vertical_off = (header_height - text_size.y) * 0.5f;
    ...
    ImVec2 col_min = im::TableGetCellBgRect( im::GetCurrentTable(), 1 ).Min;
    ImVec2 col_max = im::TableGetCellBgRect( im::GetCurrentTable(), 1 ).Max;
    float col_width = col_max.x - col_min.x;
    float col_middle = (col_min.y + col_max.y) * 0.5f;

    ImVec2 mouse_pos = im::GetMousePos();
    ...
    {
        float x = col_min.x + col_width * kl::unlerp( float( i ), horizontal_offset, horizontal_offset + horizontal_view );
        ...
        else if ( video_layer.timeline_10seconds && i % 10 == 0 )
        {
            height = 0.8f;
            color = { 165, 210, 130 };
            int res = video_layer.timeline_minutes ? (i - ((i / 60) * 60)) : i;
        ...
        float height_delta = (col_max.y - col_min.y) * (1.0f - height) * 0.5f;
...
void titian::GUISectionFuzeTimeline::render_track( ImVec2 cell_padding, float row_height, int i, float header_height, ImVec2& total_min, ImVec2& total_max )
{
    VideoLayer& video_layer = VideoLayer::get();
    ImDrawList& draw_list = *im::GetWindowDrawList();
    ...
    String text = kl::format( i + 1, ". ", track->name );
    ImVec2 text_size = im::CalcTextSize( text.data() );

    ImVec2 cursor_pos = im::GetCursorScreenPos();
    float vertical_off = (row_height - text_size.y) * 0.5f;
    ...
    if ( m_moving_media && ImRect( col_min, col_max ).Contains( im::GetMousePos() ) && im::IsMouseReleased( ImGuiMouseButton_Left ) )
    {
        ...
        float mouse_delta = im::GetMousePos().x - info.old_mouse.x;
        float offset_delta = (mouse_delta / col_width) * horizontal_view;
        float offset = (horizontal_offset - info.horiz_offset) + (info.offset + offset_delta);
        video_layer.tracks[info.track_index]->remove_media( *info.media );
        track->insert_media( offset, info.media );
    }

    float min_time = horizontal_offset;
    float max_time = horizontal_offset + horizontal_view;
    for ( auto& [offset, media] : track->media )
    {
        float media_start = offset;
        float media_end = offset + media->duration;

        float left_x_no_clamp = kl::lerp<float, false>( kl::unlerp<float, false>( media_start, min_time, max_time ), col_min.x, col_max.x );
        float left_x = kl::lerp( kl::unlerp( media_start, min_time, max_time ), col_min.x, col_max.x );
        float right_x = kl::lerp( kl::unlerp( media_end, min_time, max_time ), col_min.x, col_max.x );
        ...
        {
            float width = col_width * kl::unlerp<float, false>( media->duration, 0.0f, horizontal_view );
            float height = col_max.y - col_min.y;
            ImVec2 top_left = im::GetMousePos() - m_moving_media.value().mouse_offset;
            ImVec2 bottom_right = top_left + ImVec2( width, height );
            ImVec2 top_left_clamped = ImVec2( kl::clamp( top_left.x, col_min.x, col_max.x ), kl::max( top_left.y, total_min.y + header_height ) );
            ImVec2 bottom_right_clamped = ImVec2( kl::clamp( bottom_right.x, col_min.x, col_max.x ), kl::max( bottom_right.y, total_min.y + header_height ) );

            if ( media->type == MediaType::VIDEO )
            {
                draw_list.AddRectFilled( top_left_clamped, bottom_right_clamped, ImColor( 40, 40, 40 ) );
                float section_width = col_width * kl::unlerp<float, false>( Video::BUFFERING_LENGTH, 0.0f, horizontal_view );
                float core_width = section_width / Video::SECTION_THREAD_COUNT;
                float width_offset = 0.0f;
                for ( auto& section : media->video->buffering_sections() )
                {
                    for ( auto& [state, core] : section )
                    {
                        ...
                        ImColor color = state ? color_classify( *media ) : ImColor( 205, 120, 115 );
...
void titian::GUISectionFuzeTimeline::render_pointer( ImVec2 total_min, ImVec2 total_max )
{
    VideoLayer& video_layer = VideoLayer::get();
    ImDrawList& draw_list = *im::GetWindowDrawList();

    float x = total_min.x + (total_max.x - total_min.x) *
...
ImColor titian::GUISectionFuzeTimeline::color_classify( Media const& media )
{
