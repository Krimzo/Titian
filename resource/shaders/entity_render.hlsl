...
{
    float4 object_color;
    float4  object_data; // (has_texture?, roughness_factor, none, none)
    
    float4 camera_position;
    float4      light_data; // (sun.x, sun.y, sun.z, ambient_strength)
};
...
float4 p_shader(vs_out vs_data) : SV_Target
{
    ...
    const float specular_factor = pow(max(specular_strength, 0), 64) * (1 - object_data.y);

    const float light_intensity = ambient_factor + diffuse_factor + specular_factor;

    // Color calculations
    const float3 base_color = (object_data.x) ? entity_texture.Sample(entity_sampler, vs_data.textur) : object_color;
