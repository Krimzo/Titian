...
struct NodeStyle
{
    ...
    static std::shared_ptr<NodeStyle> sea() { return make_style( 120, 200, 175, 6.0f ); }
    ...
    static std::shared_ptr<NodeStyle> sunset() { return make_style( 240, 200, 155, 6.0f ); }
...
struct InfColors
{
    ...
    ImU32 background = IM_COL32( 35, 35, 35, 255 );
...
public:
    ...
    /* CUSTOM */
    template<typename T>
    std::shared_ptr<T> insertNode( const std::shared_ptr<T>& n );

    template<typename T>
    std::shared_ptr<T> insertNode( const ImVec2& pos, const std::shared_ptr<T>& n );
    ...
    float getScale() const { return m_context.scale(); }
    void setScroll( ImVec2 scroll ) { m_context.set_scroll( scroll ); }
    void setScale( float scale ) { m_context.set_scale( scale ); }
