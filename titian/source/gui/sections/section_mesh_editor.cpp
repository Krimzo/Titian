...
void titian::GUISectionMeshEditor::display_meshes( Scene& scene )
{
    ...
    {
        im::Text( "New Mesh" );
        String name = gui_input_continuous( "##CreateMeshInput" );
    ...
    String filter = gui_input_continuous( "Search###MeshEditor" );
...
void titian::GUISectionMeshEditor::render_selected_mesh( Mesh& mesh, Int2 viewport_size )
{
...
void titian::GUISectionMeshEditor::show_mesh_properties( Mesh* mesh )
{
    ...
    if ( im::Begin( "Mesh Properties" ) && mesh )
    {
        ...
        if ( vertex_count > 0 )
        {
            Pair window_rect = gui_window_rect();
            ...
            for ( int i = m_starting_vertex_index; i < (m_starting_vertex_index + m_vertex_display_count) && i < vertex_count; i++ )
            {
                ...
                String vertex_name = kl::format( i + 1, ". Vertex" );
                ...
                bool world_edited = im::DragFloat3( kl::format( "World##MeshEditor", i ).data(), &vertex.position.x, 0.1f );
                bool normal_edited = im::DragFloat3( kl::format( "Normal##MeshEditor", i ).data(), &vertex.normal.x, 0.1f );
                bool uv_edited = im::DragFloat2( kl::format( "UV##MeshEditor", i ).data(), &vertex.uv.x, 0.1f );
